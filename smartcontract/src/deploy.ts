...
const fsExtra = require('fs-extra'); // Importing for writing a file
...
const privateFor: any = [];
const outputFolder = args['output'] == true ? args['output'] : './build';
args['privateFor'].split(',').forEach((item: any) => privateFor.push(item));
...
const deploySmartContract = async (contractOptions: any) => {
...
const PostDeployKeeping = (abi: any, bytecode: any) => {
  try {
